...
const FaqLayout1 = ({ block }: { block: FaqBlockData }) => {
                    ...
                    block?.faqItems?.slice(0, limit === -1 ? block?.faqItems.length : limit)?.map((item, index) => {
                        return (
                            ...
                            item?.faqQuestion && item?.faqAnswer && (
                                <AccordionItem className='' value={`accordion-item-${index}`} key={index} >
                                    <AccordionTrigger className='cursor-pointer text-xl'><SafeHeading
